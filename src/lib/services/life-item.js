...
export default {
  async findAll({ q, limit, offset }) {
    let query = db.life_items.toCollection()
    if (q !== '') {
      query = query.filter(
  ...
  async trashById(id) {
    return await db.life_items.update(id, {
      deleted_at: moment()
        .utc()
        .format()
    })
  ...
  async deleteById(id) {
    return await db.life_items.delete(id)
  },
  async update(id, data) {
    ...
    updates.updated_at = moment()
      .utc()
      .format()
    return await db.life_items.update(id, updates)
  },
  async findById(id) {
    return await db.life_items.get(id)
  },
  async create(data) {
    ...
    return await db.life_items.add(insert)
  }
}
